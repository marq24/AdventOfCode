...
public class D02 {
    ...
    private static int red_max = 0;
    private static int green_max = 0;
    private static int blue_max = 0;
    ...
    private record Game(int num, boolean valid, int r, int g, int b, List<Record> moves){}
    private record Record(int num, char color){}
    ...
    private static Game parse(String line){
        ...
        int r_max=0;int g_max=0; int b_max=0;

        boolean valid = true;
        ArrayList recs = new ArrayList();
        for(String aRound: rounds){
            String[] matches = aRound.split(",");
            for(String aMatch: matches) {
                String[] aMove = aMatch.trim().split(" ");
                Record rec = new Record(Integer.parseInt(aMove[0]), aMove[1].charAt(0));
                recs.add(rec);

                if(rec.color == RED){
                    r_max = Math.max(rec.num, r_max);
                }else if(rec.color == GREEN){
                    g_max = Math.max(rec.num, g_max);
                }else if(rec.color == BLUE){
                    b_max = Math.max(rec.num, b_max);
                }

                if(valid) {
                    if (rec.color == RED) {
                        valid = rec.num <= red_max;
                    } else if (rec.color == GREEN) {
                        valid = rec.num <= green_max;
                    } else if (rec.color == BLUE) {
                        valid = rec.num <= blue_max;
                    }
                }
            }
        }
        return new Game(num, valid, r_max,g_max,b_max, recs);
    }
